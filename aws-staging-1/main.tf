...
module "aws_asg_com" {
  ...
  docker_registry_cidrs = "${data.terraform_remote_state.vpc.registry_subnet_1b_cidr} ${data.terraform_remote_state.vpc.registry_subnet_1e_cidr}"
  docker_registry_mirror = "http://${data.terraform_remote_state.vpc.registry_dns_name}"
  env = "${var.env}"
  env_short = "${var.env}"
