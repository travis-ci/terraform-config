...
sudo chmod 755 "/opt/collectd/etc/collectd.conf"
...
sudo chmod 755 "/opt/collectd/etc/collectd.conf.d/librato.conf"

#put collectd-vsphere config in /etc/default
sudo cp "/tmp/collectd-vsphere-${env}" "/etc/default/collectd-vsphere-${env}"
sudo chown root:root "/etc/default/collectd-vsphere-${env}"
sudo chmod 755 "/etc/default/collectd-vsphere-${env}"
