...
variable "worker_docker_self_image" {
  default = "travisci/worker:v3.10.0"
}
...
resource "aws_launch_configuration" "workers" {
  name_prefix       = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-"
...
resource "aws_autoscaling_group" "workers" {
  name                      = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}"
...
resource "aws_autoscaling_policy" "workers_remove_capacity" {
  name                      = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-remove-capacity"
...
resource "aws_cloudwatch_metric_alarm" "workers_remove_capacity" {
  alarm_name          = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-remove-capacity"
...
resource "aws_autoscaling_policy" "workers_add_capacity" {
  name                      = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-add-capacity"
  adjustment_type           = "ChangeInCapacity"
  policy_type               = "StepScaling"
  autoscaling_group_name    = "${aws_autoscaling_group.workers.name}"
...
resource "aws_cloudwatch_metric_alarm" "workers_add_capacity" {
  alarm_name          = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-add-capacity"
...
resource "aws_iam_role" "workers_sns" {
  name = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-sns"
...
resource "aws_autoscaling_lifecycle_hook" "workers_launching" {
  name                    = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-launching"
  autoscaling_group_name  = "${aws_autoscaling_group.workers.name}"
...
resource "aws_autoscaling_lifecycle_hook" "workers_terminating" {
  name                    = "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}-terminating"
  autoscaling_group_name  = "${aws_autoscaling_group.workers.name}"
  default_result          = "CONTINUE"
  heartbeat_timeout       = "${var.lifecycle_hook_heartbeat_timeout}"
...
resource "aws_cloudwatch_dashboard" "main" {
  dashboard_name = "${var.env}-${var.site}-${var.worker_queue}"
      ...
      "properties": {
        ...
        "metrics": [
          [
            ...
            "${var.env}-${var.index}-workers-${var.site}-${var.worker_queue}",
            {
              "color": "#2ca02c",
              "period": 30,
              "stat": "Average"
